...
def get_rental(rental_id: int, user=Depends(get_current_user)):
    ...
    conn = get_db(); cur = conn.cursor()
    cur.execute("SELECT * FROM rentals WHERE id = %s AND user_id = %s", (rental_id, user["id"]))
    rental = fetchone(cur); conn.close()
    if not rental:
...
def agent_heartbeat(req: AgentHeartbeat):
    ...
    # Gerçek GPU bilgisi varsa güncelle
    if req.gpus and len(req.gpus) > 0:
        real_gpu = req.gpus[0]
        gpu_name = real_gpu.get("name", gpu["name"])
        gpu_vram = real_gpu.get("vram_gb", gpu["vram"])
        cur.execute(
            "UPDATE gpu_listings SET status = %s, name = %s, vram = %s WHERE agent_key = %s",
            (status, gpu_name, gpu_vram, req.agent_key)
        )
    else:
